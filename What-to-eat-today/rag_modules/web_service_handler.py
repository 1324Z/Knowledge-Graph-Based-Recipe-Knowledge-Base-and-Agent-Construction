    ...
    def _handle_stream_request(self):
        ...
        try:
            ...
            def generate():
                try:
                    # 发送缓存检查状态
                    yield f"data: {json.dumps({'status': 'cache_check'})}\n\n"
                    ...
                    # 在后台线程执行路由检索，同时发送分阶段状态
                    import threading
                    route_result = {}
                    def _do_route():
                        try:
                            route_result['data'] = self.rag_system.query_router.route_query(
                                query=enhanced_query,
                                top_k=self.rag_system.config.top_k
                            )
                        except Exception as e:
                            route_result['error'] = e
                    
                    route_thread = threading.Thread(target=_do_route)
                    route_thread.start()
                    
                    # LLM查询分类约需4-5秒，5秒后发送检索状态
                    route_thread.join(timeout=5)
                    if route_thread.is_alive():
                        yield f"data: {json.dumps({'status': 'retrieving'})}\n\n"
                        route_thread.join(timeout=30)
                    
                    if 'error' in route_result:
                        raise route_result['error']
                    documents, analysis = route_result.get('data', ([], None))
