...
按需填写 `What-to-eat-today/.env` 中的 API、Neo4j、Milvus 等配置。
...
│   ├── frontend/               # Next.js 前端
│   └── kb_demo.py              # 知识库演示脚本
├── all-in-rag/data/C9/         # Neo4j / Milvus Docker 配置
└── test_*.py                   # 测试脚本
```
...
以下部分整合了原 `PROJECT.md` 的详细说明，便于在一个文档中同时查看“怎么运行”和“怎么实现”。
...
![聊天对话与分阶段状态反馈](screenshots/04_chat_conversation.png)

> 上图展示了完整的用户交互流程：用户提问"鸡蛋有哪些做法"，系统依次推送 4 个阶段状态（缓存检查→问题分析→三路混合检索→答案生成），随后流式输出包含蒸蛋羹、水煮蛋、茶叶蛋三种做法的详细回答。
...
> 首页展示推荐菜谱卡片（含图片、难度、烹饪时间、人份），提供 6 个快速提问入口，用户可点击"开始对话"进入聊天页或直接点击菜谱卡片查看详情。

**聊天页 — 空闲状态**

![聊天页空闲状态](screenshots/02_chat_empty.png)

> 聊天页空闲状态，提供 3 个快捷提问按钮，底部为输入框（支持 1000 字符，Enter 发送 / Shift+Enter 换行）。

**菜谱详情页**

![菜谱详情页](screenshots/03_recipe_detail.png)

> 菜谱详情页展示菜品图片、难度标签（中等/素食）、描述、总用时（20分钟）和份数（2人份），提供"开始烹饪"按钮进入步骤引导模式。
